...
\begin{itemize}
    ...
    \item $\texttt{PassengerEmail} \to \texttt{PassengerID}$: $R_2$ already contains \texttt{PassengerEmail, PassengerID} --- \textbf{skip}
    \item $\texttt{TrainNo, PassengerID} \to \texttt{SeatNo, BaggageQty}$: create $R_3$
\end{itemize}

\noindent $R_3$ contains \{\texttt{TrainNo, PassengerID}\}, a candidate key of $R$ --- no additional relation needed.
\begin{itemize}
    \item $R_1$(\underline{\texttt{TrainNo}}, \texttt{DepartureDate}, \texttt{ConductorID})
    \begin{itemize}
        \item \textbf{Candidate Key:} \{\texttt{TrainNo}\}
        \item \textbf{FDs Preserved:} $\texttt{TrainNo} \to \texttt{DepartureDate, ConductorID}$
    \end{itemize}

    \item $R_2$(\underline{\texttt{PassengerID}}, \texttt{PassengerEmail})
    \begin{itemize}
    ...
    \item $R_3$(\underline{\texttt{TrainNo, PassengerID}}, \texttt{SeatNo}, \texttt{BaggageQty})
    \begin{itemize}
