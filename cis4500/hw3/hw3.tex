...
\part
  ...
  \part
      ...
      In Q7, both transactions run in \texttt{SERIALIZABLE}, so READ/WRITE dependencies between them are tracked. A serialization anomaly is detected (a cycle of rw-dependencies), leading to T2's failure.

      \part
      Under \texttt{SERIALIZABLE}, READ/WRITE dependencies between transactions are tracked and a serialization anomaly leads to transaction failure. Two rw-dependencies form a cycle:

      \textbf{T2 $\xrightarrow{\text{rw}}$ T1:} T2 read \texttt{balance $\geq$ 180} at time 2 (empty). T1 later wrote rows satisfying that predicate (Madison $\to$ 180 at time 3, Noor at time 5).

      \textbf{T1 $\xrightarrow{\text{rw}}$ T2:} T1 read \texttt{balance $\geq$ 180} at time 1 (empty). T2 later wrote a row satisfying it (Luca at time 4).

      The cycle $\text{T1} \xrightarrow{\text{rw}} \text{T2} \xrightarrow{\text{rw}} \text{T1}$ means no valid serial order exists, so SSI aborts T2.
    \end{parts}
    \end{solution}
\end{questions}
\end{document}
