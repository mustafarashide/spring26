...
DESC; UPDATE accounts SET balance=1000000 WHERE user_id=1--
  ...
  \begin{parts}
    \part The Q5 transfer was transaction \textbf{id = 18}. The following payload was injected via the \texttt{order=} parameter:
    \begin{lstlisting}[language=SQL]
DESC; UPDATE transactions SET from_account=2, description='A hacked gift from Daniel Zhao. Thank me later' WHERE id=18--
    \end{lstlisting}
    The server falls back to \texttt{db.exec()}, which executes both statements. \texttt{from\_account} is now \texttt{2} and the description is updated accordingly.

    \part Bobby's transaction list \textbf{does not show} the modified row. The query filters by \texttt{from\_account = acc.id OR to\_account = acc.id} using Bobby's account id (\texttt{1}). After the UPDATE, the row has \texttt{from\_account = 2} and \texttt{to\_account = 3}, so it no longer matches and disappears from Bobby's view.

    This demonstrates that audit logs cannot be trusted in a system with SQL injection vulnerabilities. Any transaction record can be rewritten after the fact, silently redirecting attribution to a different account.
  \end{parts}
